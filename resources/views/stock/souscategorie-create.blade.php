...
@section('content')
    <div class="max-w-xl mx-auto">
        ...
        <nav class="flex items-center gap-2 text-xs text-slate-400 mb-6">
            ...
            <i class="ti ti-chevron-right text-slate-300"></i>
            <span class="text-slate-600 font-medium">{{ $description->description }}</span>
            <i class="ti ti-chevron-right text-slate-300"></i>
            <span class="text-slate-900 font-semibold">Nouvelle sous-catégorie</span>
        </nav>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="flex items-center gap-3 px-6 py-4 bg-slate-50 border-b border-slate-200">
                <div class="w-9 h-9 rounded-lg bg-emerald-100 flex items-center justify-center">
                    <i class="ti ti-tag text-emerald-600 text-lg"></i>
                </div>
                <div>
                    <h2 class="text-sm font-bold text-slate-900">Ajouter une sous-catégorie</h2>
                    <p class="text-xs text-slate-400">{{ $description->description }} — {{ $description->stock->name_stock }}</p>
                </div>
            </div>
            ...
            {{-- Formulaire --}}
            <form action="{{ route('souscategorie.store') }}" method="POST" class="px-6 py-6 space-y-5">
                @csrf
