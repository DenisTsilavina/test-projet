...
@section('content')
    <div class="card shadow">
        <div class="card-header bg-warning text-dark d-flex justify-content-between align-items-center">
            <h3 class="mb-0">Modifier l'Article : {{ $article->name }}</h3>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('articles.update', $article->id) }}" method="POST">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <label for="total_prd_finit" class="form-label fw-bold">Quantité produite</label>
                        <input type="number" class="form-control" id="total_prd_finit" name="total_prd_finit" value="{{ old('total_prd_finit', $article->total_prd_finit) }}" min="1" required>
                    </div>
                </div>

                <div class="mb-4">
                    <label for="note" class="form-label fw-bold">Note / Description</label>
                    <textarea class="form-control" id="note" name="note" rows="2">{{ old('note', $article->note) }}</textarea>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="ingredients-table">
                        ...
                        @foreach($article->ingredients as $index => $ingredient)
                            <tr>
                                <td>
                                    <select name="ingredients[{{ $index }}][description_id]" class="form-select" required>
                                        @foreach($descriptions as $d)
                                            <option value="{{ $d.id }}" {{ $d->id == $ingredient->description_id ? 'selected' : '' }}>{{ $d->libelle }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <select name="ingredients[{{ $index }}][unite_id]" class="form-select" required>
                                        @foreach($unites as $u)
                                            <option value="{{ $u->id }}" {{ $u->id == $ingredient->unite_id ? 'selected' : '' }}>{{ $u->nom }} ({{ $u->unite }})</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <input type="number" step="0.01" name="ingredients[{{ $index }}][effectif]" class="form-control" value="{{ $ingredient->effectif }}" required>
                                </td>
                                <td>
                                    <input type="number" step="0.01" name="ingredients[{{ $index }}][prix]" class="form-control" value="{{ $ingredient->prix }}" required>
                                </td>
                ...
                <div class="text-end mt-4">
                    <button type="submit" class="btn btn-warning px-5 fw-bold">Mettre à jour la recette</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        // On commence l'index après les éléments déjà générés par le Blade
        let ingredientIndex = {{ $article->ingredients->count() }};
