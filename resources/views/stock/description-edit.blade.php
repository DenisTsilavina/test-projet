    ...
    <div class="max-w-xl mx-auto">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <form action="{{ route('description.update', $description->id) }}" method="POST" class="px-6 py-6 space-y-5">
                ...
                {{-- AJOUT : ce champ manquait, alors que le contrôleur l'exige. --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">
                        Région d'origine <span class="text-rose-500">*</span>
                    </label>
                    <input type="text" name="region"
                           value="{{ old('region', $description->region) }}"
                           class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500
                              @error('region') border-rose-500 bg-rose-50/30 @else border-slate-200 @enderror text-slate-900"
                           required>
                    @error('region')
                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
